...
1. 부서번호가 10번인 사원 평균급여 보다 급여가 적은 사원의 이름과 급여(단, 급여가 높은 순 정렬)
 select ename, sal form emp where sal<(select avg(sal) from emp where deptno=10) order by sal desc;
 
2. 부서번호가 10번인 사원 평균급여 보다 급여가 적은 사원들의 부서별 평균 급여(단, 10번부서는 출력제외, 부서번호 역정렬, 급여는 반올림할 것)
